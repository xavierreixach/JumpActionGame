...
class GameScreen(private val mGame: JumpActionGame) : ScreenAdapter() {
    ...
    init {
        ...
        mPrefs = Gdx.app.getPreferences("jp.techacademy.umiya.fukuyama.jumpactiongame")
        mHighScore = mPrefs.getInteger("HIGHSCORE", 0)

        mSound = Gdx.audio.newSound(Gdx.files.internal("data/mysound.mp3"))
    ...
    private fun createStage() {
        ...
        while (y < WORLD_HEIGHT - 5) {
            ...
            if (mRandom.nextFloat() > 0.7f) {
                val enemy = Enemy(enemyTexture, 20, 20, 72, 72)
